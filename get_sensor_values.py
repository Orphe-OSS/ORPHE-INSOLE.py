...
def on_disconnect():
    print("Disconnected from the ORPHE INSOLE device.")
...
async def main():
    ...
    # 接続するデバイスを指定する場合はコアモジュールのaddressをconnect()の引数に文字列として渡してください。addressを知りたい場合はscan.pyを実行して該当するデバイスのaddressを確認してください。文字列が空の場合はSERVICE UUIDがORPHE INSOLEと合致する最初に見つかったデバイスに接続します。
    if not await orphe.connect():
        return

    # ジャイロ、加速度、圧力を200Hzでストリーミング(1,3,4のいずれかを指定)
...
#  以下は色々記述していますが，Ctrl+Cでプログラムを終了した場合にきれいに終了処理するためのものです．最悪 asyncio.run(main()) だけでも動きます．
