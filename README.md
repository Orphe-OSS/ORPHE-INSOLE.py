...
### センサ値の換算について（2026-07 更新）

物理値への換算を2箇所修正しました。正規化値（`got_acc_callback` / `got_gyro_callback` で得られる `raw/32768`）と加速度の物理値は変更していません。

| 対象 | 修正前 | 修正後 | 理由 |
|---|---|---|---|
| ジャイロ物理値 `converted_gyro`[deg/s] | `raw / 32768 * レンジ` | `raw * レンジ * 0.000035` | IMU(LSM6DSOX)の実感度は ±250/500/1000/2000 dps で 8.75/17.5/35/70 mdps/LSB。理想Q15換算では約12.8%小さい値になっていた |
| クォータニオン `quat` | `raw / 32768`（Q15） | `raw / 16384`（Q14） | FWはQ14で符号化しており、Q15ではノルムが約0.5に潰れオイラー角も圧縮されていた |

* 過去に記録したジャイロ物理値は **`1.14688` 倍**（= `32768 × 0.000035`）すれば新しい換算に合わせられます。
* 実機での確認は `python verify_scale_fix.py` を実行してください（quatノルムが1.0付近になるか、90度回転させたときのジャイロz積分角とquat yaw変化が一致するかを判定します）。
* 換算式の単体テストは BLE 不要で実行できます。
